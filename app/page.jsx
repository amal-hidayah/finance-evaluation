...
export default function Home() {
  ...
  const loadingMessages = [
    ...
    "Menghitung rasio pemasukan vs pengeluaran...",
    "Menyiapkan evaluasi finansial...",
    "Membuat rencana anggaran...",
    "Menyusun respons evaluasi..."
  ];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(error);
      alert('Gagal menganalisis data. Coba lagi.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className={styles.container}>
      <div ref={dashboardRef}>
        ...
        <header className={`${styles.mainHeader} ${styles.mobileHeader} ${styles.exportHeader}`}>
          <h1 className={styles.title}>Ruthless Ledger</h1>
          <p className={styles.subtitle}>Ms. Ledger is ready to review your poor financial choices.</p>
        </header>

        <div className={styles.auditLayout}>
          {/* Left Side: The Auditor Character */}
        <div className={styles.characterPanel}>
          <img src="/logo.webp" alt="Ms. Ledger" className={styles.avatarLarge} />
          
          <div className={styles.speechBubble}>
            {!isLoading && !result && "Tuliskan detail pengeluaran Anda, dan mari kita lihat seberapa buruk keputusan finansial Anda bulan ini."}
            {isLoading && loadingMessage}
            {result && !isLoading && <TypewriterText text={result.roast} />}
          </div>
        </div>

        {/* Right Side: The Content / Input */}
          ...
          <main>
            {!isLoading && !result && (
              <section className={`${styles.inputSection} glass-panel`}>
                <label htmlFor="finance-input" className={styles.promptLabel}>
                  Deskripsikan Pemasukan dan Pengeluaran Anda:
                </label>
                <textarea
                  id="finance-input"
            ...
            {result && !isLoading && (
              <div className={styles.dashboard}>
                {/* Savage Score Badge */}
                {result.savage_score && (
                  <div className={`${styles.card} glass-panel`} style={{ padding: '1.5rem', textAlign: 'center', borderColor: result.savage_score.includes('A') || result.savage_score.includes('B') ? '#4ade80' : 'var(--accent-red)' }}>
                    ...
                    <div style={{ fontSize: '1.8rem', fontWeight: '900', color: result.savage_score.includes('A') || result.savage_score.includes('B') ? '#4ade80' : 'var(--accent-red)' }}>
                      {result.savage_score}
                    </div>
                  </div>
                )}
                ...
                <div id="capture-buttons" style={{ display: 'flex', gap: '1rem', width: '100%', marginTop: '2rem' }}>
                  ...
                  <button 
                    ...
                    style={{ flex: 1, margin: 0, background: '#4ade80', color: '#000' }}
                  >
                    <Download size={20} />
